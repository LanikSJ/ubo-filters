...
sub output_section {
    ...
    if (@$lines) {
        ...
        log_info("Sorted " . scalar(@sorted_lines) . " lines in $section_type section");
    }
...
sub create_backup {
    ...
    if (-e $backup_file) {
        log_warning("Backup file '$backup_file' already exists, will overwrite");
    }
...
sub process_file {
    ...
    while (my $line = <$input_fh>) {
        ...
        if ($line =~ /^(?:[!\[]|[#|;]\s)/) {
            ...
            if (@$current_section) {
                output_section($current_section, $output_fh, "section $section_count");
                $section_count++;
            }
...
sub main {
    ...
    # Summary
    if ($total_files > 1) {
        log_info("Summary: $success_count/$total_files files processed successfully");
    }
